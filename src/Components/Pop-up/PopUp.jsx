...
const PopUp = ({cartList, handleCheckOut, total}) => {
    ...
    return (
        <div className="pop-up">
            <div className="content">
                    ...
                    <div className="items">
                        {
                            cartList.map((item, index)=>{
                                const imageKey = `image-${((item.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "")).toLowerCase()).replace(" ","-")}-thumbnail`
                                const imageSrc = allImages[imageKey]

                                return(
                                    <CheckoutItem key={index} item={item} imageSrc={imageSrc}/>
                                )
                            })
                        }
                    </div>
                ...
                <button className="confirm-btn clickable" onClick={()=>{
